...
use Ti\Helpdesk\App\System\File;
use Ti\Helpdesk\App\System\Security\Encryption;
use Ti\Helpdesk\App\System\Session;
/*
|--------------------------------------------------------------------------
| Run The Application
|--------------------------------------------------------------------------
...
if (!function_exists('user')) {
    ...
    function user()
    {
        $user = \Ti\Helpdesk\App\System\Auth\AuthenticateUser::user();

        if (!$user) {
            $user = \Ti\Helpdesk\App\System\Auth\AuthenticateUser::remembered(
                \Ti\Helpdesk\App\System\Request::createFromGlobals()
            );
        }
...
if (!function_exists('setCookie')) {
    ...
    function setCookie($name, $value = null, $expire = 0)
    {
        ...
        $response->headers->setCookie(new \Ti\Helpdesk\App\System\Cookie($name, $value, $expire));
        $response->send();
...
if (!function_exists('readCookie')) {
     ...
     * Read cookie value.
     *
     * @param Ti\Helpdesk\App\System\Request $request
     * @param $name
     *
     * @return mixed
     */
    function readCookie(Ti\Helpdesk\App\System\Request $request, $name)
    {
